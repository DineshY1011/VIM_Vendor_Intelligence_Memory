...
Handles:
  1. Document text extraction (.txt, .pdf)
  2. LLM fact extraction via Groq
  3. Drift classification against baseline
  4. Writing to Hindsight via memory.py

Two upload modes:
  - "baseline"  : Process as the original contract. Writes {vendor_id}:baseline.
  - "normal"    : Process as a post-contract document. Appends to {vendor_id}:events.
"""
...
def extract_baseline(text: str, filename: str) -> Optional[dict]:
    """Use Groq to extract baseline fields from a contract document."""
...
def classify_drift(baseline: dict, fact: dict) -> Optional[dict]:
    """Compare a new fact against the vendor baseline and classify drift."""
    prompt = (
...
def is_duplicate(event: dict, existing_events: list) -> bool:
    """
    Check duplicate per §3.5: compare fact_type + value + date.
    Discard if all three match an existing event.
    """
...
async def process_baseline_doc(
    ...
    Extracts the six baseline fields and writes {vendor_id}:baseline to Hindsight.
    Returns: {"ok": bool, "baseline": dict, "error": str}
    """
    ...
    if not baseline:
        return {"ok": False, "baseline": None, "error": "Could not extract baseline fields from document."}

    # Ensure all six fields present
...
async def process_normal_doc(
    ...
    Extracts facts, runs drift classification, appends to {vendor_id}:events.
    Returns: {"ok": bool, "events_added": list, "events_skipped": list, "error": str}
    """
    ...
    if not extraction or not extraction.get("facts"):
        return {"ok": False, "events_added": [], "events_skipped": [],
    ...
    for fact in facts:
        ...
        # Map old fact_type names to new spec if needed
        ...
        ft_map = {
            ...
            "incident":    "incident",
            "baseline":    None,  # skip — belongs in baseline mode
        }
        ...
        # Run drift classification if we have a baseline
        severity = 1
        drift_result = None
        ...
        # Build event object per §3.1 schema
        event = {
            "date":       doc_date,
